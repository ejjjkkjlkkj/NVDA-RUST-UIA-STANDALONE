...
$textEvents = @(
    $screenReaderLines |
        Where-Object {
            $_ -match '^(TEXT_CHANGED/UIA_20015|TEXT_SELECTION_CHANGED/UIA_20014) #' -and
            $_ -match 'Framework=Chrome'
        }
)

$semanticRoles = [ordered]@{}
foreach ($role in @('editable_text', 'check_box', 'combo_box', 'button', 'link')) {
    $matches = @(
        $screenReaderLines |
            Where-Object { $_ -match '^SEMANTIC_FOCUS #' -and $_ -match "\| role=$role(?:\s*\||$)" }
    )
    $semanticRoles[$role] = $matches.Count
    if ($matches.Count -lt 1) {
        throw "Semantic adapter did not expose expected web role '$role'"
    }
}

$detailedSpeech = @(
    $screenReaderLines |
        Where-Object { $_ -match '^SPEECH_OUTPUT #\d+ = PASS \| Detailed\s*$' }
)
if ($detailedSpeech.Count -lt 1) {
    throw 'Changing the web combo box to Detailed did not produce value speech'
}

$summary = [ordered]@{
    schemaVersion = 2
    platform = 'windows'
    scenario = 'edge-web-semantic-keyboard-e2e'
    pass = $true
    ...
    detailedComboSpeechOutputs = $detailedSpeech.Count
...
"EDGE_WEB_SEMANTIC_E2E = PASS | keyboard_actions=$($keyboardActions.Count) | chromium_focus=$($chromeFrameworkLines.Count) | semantic_roles=5 | detailed_speech=$($detailedSpeech.Count) | value_events=$($valueEvents.Count) | toggle_events=$($toggleEvents.Count) | text_events=$($textEvents.Count) | mouse_calls=0"
